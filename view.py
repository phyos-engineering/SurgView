...
class UIReader:
	...
	def extract_text(self, extracted_image,
		...
		if flag == "bottom_buttons_template":
			...
			threshold = cv.threshold(src=gray_image, thresh=127, maxval=255, type=cv.THRESH_BINARY_INV)[1]
		...
		extracted_text = None
		if threshold is None:
			extracted_text = pytesseract.image_to_string(image=extracted_image.copy(), config='--psm 6')
		if threshold is not None:
			extracted_text = pytesseract.image_to_string(image=threshold.copy(), config='--psm 3')
	...
	def contour_matching(self, show_results: bool, template_flag: str):
		...
		interface = cv.imread(self.source_filepath)  # -> Switch out for current_view once everything is ready
...
def main():
	...
	ap.add_argument("--source", nargs='?', default="interface_assets/steris/home_page_templates/home_page_root.jpg",
					type=str, help="Source image where mapping will be performed.")
