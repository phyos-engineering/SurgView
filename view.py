...
class UIReader:
    def __init__(self):
        ...
        self.old_view = None
        logging.basicConfig(
            filename=self.session_logger.log_path + "/session.log", level=logging.DEBUG
        )
    ...
    def check_diff(self):
        ...
        err = np.sum(
            (self.old_view.astype("float") - self.current_view.astype("float")) ** 2
        )
    ...
    def extract_text(
        self, extracted_image, flag: str
    ) -> str:  # TO DO: Pass filepath or mat?
        """
        ...
        if flag == "bottom_buttons_template":
            enlarged_image = cv.resize(src=extracted_image, dsize=(0, 0), fx=9, fy=9)

            gray_image = cv.cvtColor(enlarged_image.copy(), cv.COLOR_BGR2GRAY)
            threshold = cv.threshold(
                src=gray_image, thresh=127, maxval=255, type=cv.THRESH_BINARY_INV
            )[1]
        ...
        print(
            "Extracting Text: {} - Elapsed Time: {}s".format(
    ...
    def contour_matching(self, show_results: bool, template_flag: str):
        ...
        self.show_result(
            "Grayscale image with a threshold applied", threshold, show_results
        )
        ...
        )  # Only grab contour with largest area
        ...
        interface = (
            self.current_view
        )  # cv.imread(self.source_filepath)  # -> Switch out for current_view once everything is ready
        ...
        self.show_result(
            "Interface Image After Grayscale Conversion", interface_gray, show_results
        )
        ...
        cv.drawContours(
            interface_contours, contours, contourIdx=-1, color=(0, 255, 0), thickness=3
        )
        self.show_result(
            "Detected Contours In Interface", interface_contours, show_results
        )
    ...
    def map_interface(self, show_flag=False):
        ...
        self.template_filepath = (
            "interface_assets/steris/home_page_templates/main_window.jpg"
        )
        self.session_logger.record_picture(
            self.contour_matching(
                show_results=show_flag, template_flag="main_window_template"
            )
        )
        # Find Sources
        print("FINDING SOURCES")
        self.template_filepath = (
            "interface_assets/steris/home_page_templates/vitals_camera.jpg"
        )
        self.session_logger.record_picture(
            self.contour_matching(
                show_results=show_flag, template_flag="sources_template"
            )
        )
        ...
        self.template_filepath = (
            "interface_assets/steris/home_page_templates/surgical_display_1.jpg"
        )
        self.session_logger.record_picture(
            self.contour_matching(
                show_results=show_flag, template_flag="destinations_template"
            )
        )
        ...
        self.session_logger.record_picture(
            self.contour_matching(
                show_results=show_flag, template_flag="bottom_buttons_template"
            )
        )

        # Find Take Picture and Record Video Buttons, End Case and Begin Case
        self.template_matching()

        # Find Rectangular Bottom Buttons
        print("FINDING RECT BUTTONS")
        self.template_filepath = (
            "interface_assets/steris/home_page_templates/begin_case.jpg"
        )
        self.session_logger.record_picture(
            self.contour_matching(
                show_results=show_flag, template_flag="bottom_buttons_template"
    ...
    def template_matching(self):
        ...
        for image in os.listdir(targets_dir):
            ...
            contours = cv.findContours(
                partial_image.copy(), cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE
            )
            contours = contours[0] if len(contours) == 2 else contours[1]
            ...
            mask = np.zeros(
                (widget_img.shape[0], widget_img.shape[1], 3), dtype=np.uint8
            )
        ...
        print(
            "Average Processing Time Per Template: {}s".format(
