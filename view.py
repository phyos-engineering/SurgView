...
class UIReader:
    ...
    def record_session(self):  # This has to run on a seperate thread
    ...
    def stream_session(self):  # This has to run on a seperate thread
        return "frame"
    ...
    @staticmethod
    def extract_text(extracted_image,
        ...
        if flag == "destinations_template":
            threshold = cv.threshold(src=gray_image,
                                     thresh=127,
                                     maxval=255,
                                     type=cv.THRESH_BINARY + cv.THRESH_OTSU)[1]
        ...
        cleaned_text = ''.join(e for e in extracted_text
                               if e.isalnum()).lower()
        end_time = time.time()
    ...
    def map_interface(self):
        ...
        self.contour_matching(show_results=False,
